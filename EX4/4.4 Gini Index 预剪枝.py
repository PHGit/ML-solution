...
class DecisionTree(object):
    ...
    def pre_test(data_set, data_test,best_index, feat_set):
        ...
        for feat in feat_set:
            sub_data_set = data_set.filter_value(best_index, feat)  # 划分数据集并返回子数据集
            sub_data_test = data_test.filter_value(best_index, feat)
            # print sub_data_set.class_most()
            # print sub_data_test.class_set
            error += DecisionTree.test_majority(sub_data_set.class_most(), sub_data_test)
        print error
        return error
    ...
    def tree_generate(self, data_set, data_test, feature_dict):
        ...
        print feat_set
        # print DecisionTree.pre_test(data_set,data_test,best_feature_index,feat_set),DecisionTree.test_majority(data_set.class_most(),data_test)
        if DecisionTree.pre_test(data_set,data_test,best_feature_index,feat_set) >= \
                DecisionTree.test_majority(data_set.class_most(),data_test):   #当划分后的错误率 大于或者等于 不划分的错误率 不划分
            node.isLeaf = True  # 标记新节点为叶子节点
            node.decision = data_set.class_most()  # 类别为父数据集中样本数最多的类
            return node
